...
void makeAction(void){
	//std::cout<<"in makeAction"<<std::endl;
	std::ofstream permission;
	std::ifstream contiornot;
	permission.open("./src/dash/model/algorithms/data/permission");
	permission<<1;
	permission.close();
	char flag[4];
	while(1)
	{
		contiornot.open("./src/dash/model/algorithms/data/permission");
		contiornot >> flag;
		if (flag[1]=='0'){
			break;
		}
...
//
  ...
  for(double t=5.3;t<=stopTime;t+=5.0){
	//std::cout<<t<<std::endl;
  	Simulator::Schedule(Seconds(t),makeAction);
  }
