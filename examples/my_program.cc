...
std::ifstream bwfile;
double m_timer_value;
double m_timer_value_next;
double linkRate_num;
double linkRate_num_next;
...
main(int argc, char *argv[])
{
  ...
  bwfile.open("./src/dash/examples/tracefile");
...
BandwidthTrace (void)
{
  ...
  bwfile>>m_timer_value>>linkRate_num;
  position = bwfile.tellg();

  bandwidth = std::to_string(linkRate_num) + "Mbps";
  //cout<<bandwidth<<Simulator::Now()<<endl;
  ns3::Config::Set("/NodeList/1/DeviceList/0/$ns3::PointToPointNetDevice/DataRate", StringValue(bandwidth));
  

  
  bwfile>>m_timer_value_next>>linkRate_num_next;
  bwfile.seekg(position);

  m_timer_value = m_timer_value_next - m_timer_value;
  //std::cout << "BandwidthTrace bandwidth " << bandwidth << m_timer_value << std::endl;

  if (m_timer_value_next>200.0)
  {
    return;
  }
...
ScheduleBw (void)
{
  Simulator::Schedule(Seconds(m_timer_value), &BandwidthTrace);
}
