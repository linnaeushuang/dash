...
'''
this code for fixBandWidth

'''
def outputFI(SAMPLE_START=50,SAMPLE_END=150,BANDWIDTH=5000000.0):
    realPath=os.path.realpath(__file__)[:-5]
    #print(realPath)
    #print(realPath+'bb')
    docName=[realPath+'bb',realPath+'mpc',realPath+'mpcfast',realPath+'pensieve']
    K=len([name for name in os.listdir(docName[0]) if name[:3]=='log'])
    #print(K)
    FairIndex=[]
    EfficiencyIndex=[]
    StabilityIndex=[]
    ...
    for docindex in range(len(docName)):
        ...
        for sampleIndex in range(SAMPLE_END-SAMPLE_START):
            ...
            sumb_x_t=abs(sumb_x_t-BANDWIDTH)/BANDWIDTH
            l.append(sumb_x_t)
