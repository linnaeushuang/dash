...
#include <string>
std::string TRACE_NAME_MPCFAST="null";
int USERS_MPCFAST=0;
...
namespace ns3
{
      ...
      Time & delay, Time m_segmentFetchTime, int id, Time currDt,uint32_t m_segmentId)
  {
	  ...
	  if(mpcPBlen>=5){
		  mpcPBfront++;
		  mpcPastBandwidth[(mpcPBfront+mpcPBlen-1)%5]=currRate/(2000000*m_segmentFetchTime.GetSeconds());
	  }
	  else{
		  mpcPBlen++;
		  mpcPastBandwidth[(mpcPBfront+mpcPBlen-1)%5]=currRate/(2000000*m_segmentFetchTime.GetSeconds());
	...
	*/
			...
			for(int b3=0;b3<6;b3++){
				...
				for(int b4=0;b4<6;b4++){
					...
					for(int b5=0;b5<6;b5++){
						...
						for(int i=0;i<5;i++){
							double downloadTime=(bitRate[futureList[i]]/2000000.0)/harmonicBandwidth;
	...
	if(log_output.is_open()){
		...
		log_output.open("./src/dash/model/algorithms/results/log_"+TRACE_NAME_MPCFAST.substr(7)+"_"+GetTypeId().GetName().substr(5,GetTypeId().GetName().length()-11)+"_"+std::to_string(USERS_MPCFAST)+"_"+std::to_string(id));
		s_real_buffer_output.open("./src/dash/model/algorithms/results/realbuffer_"+TRACE_NAME_MPCFAST.substr(7)+"_"+GetTypeId().GetName().substr(5,GetTypeId().GetName().length()-11)+"_"+std::to_string(USERS_MPCFAST)+"_"+std::to_string(id));
		//log_output<<Simulator::Now().GetSeconds()<<" "<<bufferLevel<<" "<<currRate<<" "<<m_segmentFetchTime.GetSeconds()<<std::endl;
		log_output<<nowtime<<" "<<currRate<<" "<<bufferLevel<<" "<<rebuf<<" "<<chunk_size<<" "<<lastdownloadtime<<" "<<reward<<std::endl;
