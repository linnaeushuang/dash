...
def main( usersnumber,p1=1,p2=1,p3=1):
    ...
    logging=[open(LOG_FILE+'time'+str(i)+'_'+str(p1)+'_'+str(p2)+'_'+str(p3),'wb') for i in range(usersnumber)]
        ...
        with open(DATA_PATH + '/permission') as enable:
            ...
            if key == '1':
                ...
                if time_step>1:
                    ...
                    allactionlist.append(read_action)
                    if time_step % updateStep == 0:
                        maddpg.updateNetwork(np.array(allstatelist),np.array(allactionlist),np.array(allrewardlist))
                        maddpg.saveModel(time_step)
                        del allstatelist[:]
                        del allrewardlist[:]
                        del allactionlist[:]
