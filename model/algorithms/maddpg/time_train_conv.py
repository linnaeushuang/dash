...
from Maddpg_conv import Sakuro
import torch
...
def main( usersnumber,p1=1,p2=1,p3=1):
    ...
    logging=[open(LOG_FILE+'time'+str(i),'wb') for i in range(usersnumber)]
            ...
            if key == '1':
                ...
                for m_id in range(usersnumber):
                    ...
                    with open(DATA_PATH+'/rebufftime'+str(m_id)) as f:
                        for m_id,line in enumerate(f):
                            rebuffer[m_id]=float(line.split()[0])
                    state[usersnumber*(0)+m_id,-1]=bitrate[m_id]/1000000.0
                ...
                if time_step>1:
                    ...
                    action=np.reshape(maddpg.actionSelect(state),usersnumber)
                    #print(action)
                    for m_id in range(usersnumber):
                        reward.append(p1*np.log2(bitrate[m_id]/1000000.0+2.0)-p2*fairness-p3*rebuffer[m_id])
                        #reward.append(p1*bitrate[m_id]/1000000.0-p2*fairness-p3*rebuffer[m_id])
                        #print(str(action) + '\t'+str(rebuffer)+'\t'+str(reward))
                        output_file = open(DATA_PATH + '/predict' + str(m_id),'a')
                        bitrate[m_id]=VIDEO_BIT_RATE[int(action[m_id])]*1000
                        #print(bitrate[m_id])
                        output_file.write(str(bitrate[m_id])+'\n')
                        output_file.close()
                        logging[m_id].write(str(time_step)+'\t'+str(VIDEO_BIT_RATE[int(action[m_id])]*1000)+'\t'+str(state[usersnumber*2+m_id,-1])+'\t'+str(state[usersnumber*1+m_id,-1])+'\t'+str(rebuffer[m_id])+'\t'+str(reward[m_id])+'\n')
                        logging[m_id].flush()
                    allstatelist.append(state)
                    allrewardlist.append(reward)
                    allactionlist.append(action)
...
if __name__ == '__main__':
    users = sys.argv[1]
    main(int(users),float(sys.argv[2]),float(sys.argv[3]),float(sys.argv[4]))
